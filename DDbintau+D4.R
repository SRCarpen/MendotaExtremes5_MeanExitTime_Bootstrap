...
DDbins = function(Xvar,bw,ntau,nbin) {
  ...
  D4 = rep(0,na)
  nbin = rep(0,na)
  SD = rep(1,na) # variate for standard deviation
  # calculate moments, D1, D2 by bin
  for(ib in 1:na) {
    Xb = subset(Xdtf,subset = (Xdtf$bin==ib) )
    X01[ib,1] = ib
    X01[ib,2] = mean(Xb[,1],na.rm=T) # Mean of X0
    nxb = length(Xb[,1])
    nbin[ib] = nxb
    ...
    dx4 = dx2*dx2
    M1 = colMeans(dx1,na.rm=T)
    M2 = colMeans(dx2,na.rm=T)
    M4 = colMeans(dx4,na.rm=T)
    SD[ib] = sd(dx1[,1],na.rm=T) # note dX1 already excludes the time index
    ...
    xvec2 = 2*xvec # D2*(2*tau) is slope*x
    # 2021-05-10: does 0.5 over-correct, since tau is already multiplied by 2?
    #D2[ib] = 0.5*( 1/(t(xvec2)%*%WD2%*%xvec2) )*(t(xvec2)%*%WD2%*%yvec)
    # either remove 0.5 or use xvec instead of 2*xvec=xvec2
    D2[ib] = 0.5*( 1/(t(xvec)%*%WD2%*%xvec) )*(t(xvec)%*%WD2%*%yvec)
    D2[ib] = ifelse(nxb <= 1 || D2[ib]<=0,D2[(ib-1)],D2[ib]) # adjust for bins with only 1 observation
    # D4
    WD4 = diag( rep(1,ntau) )
    yvec = M4
    D4[ib] = (1/24)*( 1/(t(xvec)%*%WD4%*%xvec) )*(t(xvec)%*%WD4%*%yvec)
    D4[ib] = ifelse(nxb <= 1 || D4[ib]<=0,D4[(ib-1)],D4[ib]) # adjust for bins with only 1 observation
   }
  
  # calculate SD of Xdtf[,2]
  SDall = sd(Xdtf[,2],na.rm=T)
  ...
  sigmas = ksmooth(x=X01[,2],y=sigma,kernel='normal',bandwidth=bw,
                   x.points=bin.mid)
  D4s = ksmooth(x=X01[,2],y=D4,kernel='normal',bandwidth=bw,
                x.points=bin.mid)
  
  outlist = list(D1s,D2s,sigmas,D4s,bin.mid,D1,D2,sigma,D4,SD,SDall) 
  print('output list',quote=F)
  print('list(D1s,D2s,sigmas,D4s,bin.mid,D1,D2,sigma,D4,SD,SDall)',quote=F)
  return(outlist)
}
